...
import {valueOrDefault} from 'node_modules/chart.js/helpers';
var _seed = Date.now();
...
export function points(this: any, config: any) {
  const xs = this.numbers(config);
  const ys = this.numbers(config);
  return xs.map((x: any, i: string | number) => ({x, y: ys[i]}));
}

export function bubbles(this: any, config: any) {
  return this.points(config).map((pt: { r: any; }) => {
